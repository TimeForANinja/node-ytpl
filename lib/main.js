...
const main = module.exports = async(linkOrId, options, rt = 3) => {
  ...
  // Parse videos
  const itemSectionArray = parsed.json.contents
    .twoColumnBrowseResultsRenderer.tabs[0].tabRenderer.content
    .sectionListRenderer.contents;

  let rawVideoList = [];

  itemSectionArray.forEach(x => {
    x.itemSectionRenderer.contents.every(y => {
      if (y.playlistVideoListRenderer) {
        rawVideoList = y.playlistVideoListRenderer.contents;
