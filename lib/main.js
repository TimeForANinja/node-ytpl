...
// Checks for a (syntactically) valid ID
main.validateID = id => {
  if (!id || typeof id !== 'string') {
    return false;
  }
  if (PLAYLIST_REGEX.test(id) || ALBUM_REGEX.test(id)) {
    return true;
  }
  if (CHANNEL_REGEX.test(id)) {
    return true;
  }
  return false;
};
...
main.validateURL = url => {
  ...
  const parsed = new URL(url, BASE_PLIST_URL);
  if (!YT_HOSTS.includes(parsed.host)) return false;
  if (parsed.searchParams.has('list')) {
    const listParam = parsed.searchParams.get('list');
    return main.validateID(listParam);
  }
  return false;
};

// Checks for a (syntactically) valid ID or URL
main.validate = linkOrId => {
  // Validate inputs
  if (typeof linkOrId !== 'string' || !linkOrId) {
    return false;
  }
  return main.validateID(linkOrId) || main.validateURL(linkOrId);
};
