...
const main = module.exports = async(linkOrId, options, rt = 3) => {
  ...
  const resp = {
    ...
    estimatedItemCount: info.stats.length === 3 ? UTIL.parseIntegerFromText(info.stats[0]) : -1,
    views: UTIL.parseIntegerFromText(info.stats[info.stats.length === 3 ? 1 : 0]),
    thumbnails,
    bestThumbnail: thumbnails[0],
    lastUpdated: UTIL.parseText(info.stats[info.stats.length === 3 ? 2 : 1], ''),
