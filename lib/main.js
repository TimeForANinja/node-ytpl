...
const main = module.exports = async(linkOrId, options) => {
  ...
  const ref = BASE_PLIST_URL + plistId + '&' + QS.encode(opts.query);
  ...
  const info = parsed.json.sidebar.playlistSidebarRenderer.items[0].playlistSidebarPrimaryInfoRenderer;
  ...
  // Parse videos
  const rawVideoList = parsed.json.contents.twoColumnBrowseResultsRenderer.tabs[0].tabRenderer.content.sectionListRenderer.contents[0].itemSectionRenderer.contents[0].playlistVideoListRenderer.contents;
