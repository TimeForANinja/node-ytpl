...
const AUTHOR_ALBUM_REGEXP = /<ul class="pl-header-details"><li>(.*?(?=<\/li>))<\/li><li>(.*?)(?=<\/li>)<\/li><li>(.*?(?=<\/li>))<\/li>/; // eslint-disable-line max-len
...
exports.getGeneralInfo = (body, plistId) => {
  ...
  let authorMatch = importantTxt.match(AUTHOR_REFLINK_REGEXP);
  ...
  const ISALBUM = ALBUM_REGEX.test(plistId);
  if (ISALBUM) {
    authorMatch = importantTxt.match(AUTHOR_ALBUM_REGEXP);
    authorMatch.unshift(null);
  }
  return {
    id: plistId,
...
exports.getPlaylistId = (link, callback) => {
  ...
  if (PLAYLIST_REGEX.test(link) || ALBUM_REGEX.test(link)) {
    return callback(null, link);
  }
  if (CHANNEL_REGEX.test(link)) {
    return callback(null, `UU${link.substr(2)}`);
  }
...
exports.getClientVersion = string => {
