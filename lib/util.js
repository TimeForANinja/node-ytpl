...
exports.getPlaylistId = async link => {
  if (typeof link !== 'string') {
    return Error('The link has to be a string');
  }
  ...
  if (Object.hasOwnProperty.call(parsed.query, 'list')) {
    ...
    return Error('invalid list query in url');
  }
  ...
  if (maybeType === 'channel') {
    if (CHANNEL_REGEX.test(maybeId)) {
      ...
      return Error(`Unable to find a id in ${link}`);
    }
  } else if (maybeType === 'user') {
    return await exports.userToChannelUploadList(maybeId);
  } else {
    return Error(`Unable to find a id in ${link}`);
  }
};
...
exports.userToChannelUploadList = user => {
  return new Promise((resolve, reject) => {
    ...
    const req = HTTPS.get(`https://www.youtube.com/user/${user}`, resp => {
      ...
      resp.on('end', () => {
        ...
        if (channelMatch) return resolve(`UU${channelMatch[1]}`); // eslint-disable-line consistent-return
        reject(new Error(`unable to resolve the user: ${user}`)); // eslint-disable-line consistent-return
      });
    });
    req.on('error', err => {
      if (finished) return;
      finished = true;
      reject(new Error(`request failed with err: ${err.message}`));
    });
  });
};
