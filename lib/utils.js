 ...
 *
 * Property of https://github.com/fent/node-m3u8stream/blob/master/src/parse-time.ts
 */
const timeFormat = /^(?:(?:(\d+):)?(\d{1,2}):)?(\d{1,2})(?:\.(\d{3}))?$/;
const timeUnits = { s: 1000, m: 60000, h: 3600000 };
const numberFormat = /^\d+$/;
exports.humanTime = time => {
  if (typeof time === 'number') { return time; }
  if (numberFormat.test(time)) { return +time; }
  const firstFormat = timeFormat.exec(time);
  if (!firstFormat) return NaN;
