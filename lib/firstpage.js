...
var nextpage_regexp_l = /\"(\/browse_ajax\?action_continuation=1&amp;continuation=([^"]+))\"/;
...
exports.get_firstpage = function(playlist_id, options, callback) {
  var request = https.get('https://www.youtube.com/playlist?list=' + playlist_id + '&hl=en&disable_polymer=true', function(resp) {
    ...
    resp.on('end', function() {
      ...
      // parse videos
      var playlistitems = util.get_video_containers(resp_string);
      ...
      if(nextpage_link) response.nextpage = util.remove_html(nextpage_link[1]);
