...
var ignorable_errors = [
	'Try out a fresh look for YouTube. <a href="/new">Learn more</a>.' // No longer required when polymer is disabled.
];

exports.get_firstpage = function(playlist_id, options, callback) {
	var request = https.get(appendQuery('https://www.youtube.com/playlist?list=' + playlist_id + '&hl=en', 'disable_polymer=1'), function(resp) {
		...
		resp.on('end', function() {
			...
			let error_match = resp_string.match(error_regexp_g);
			if(error_match) {
				var errors = error_match
					.map(function(item) { return item.match(error_regexp_l)[1] })
					.filter(function(err) {
						return !ignorable_errors.includes(err);
					});
