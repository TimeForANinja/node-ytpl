...
module.exports = (playlistID, options, callback) => {
  const request = HTTPS.get(`https://www.youtube.com/playlist?list=${playlistID}&hl=en&disable_polymer=true`, resp => { // eslint-disable-line consistent-return, max-len
    ...
    resp.on('end', () => { // eslint-disable-line consistent-return
      ...
      response.items = UTIL.getVideoContainers(respString)
        ...
        .filter((item, index) => options.limit ? options.limit > index : true);
      ...
      // Update limit
      if (options.limit) options.limit -= response.items.length;
